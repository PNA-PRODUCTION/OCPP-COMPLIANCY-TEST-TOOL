...
export const TC_016_CS_SEQUENCE = [
  {
    step: 1,
  ...
  {
    step: 6,
    ev: "Send Reset.req (type = Soft)",
    evse: "Send Reset.conf",
    description: "Send Reset.req with type='Soft'. Verify that Reset.conf is received with status 'Accepted'."
  },
  {
    step: 7,
    ev: "Send StopTransaction.conf",
    evse: "Send StopTransaction.req (reason = SoftReset)",
    description: "Verify that StopTransaction.req is received with reason 'SoftReset'. Verify that StopTransaction.req contains the same IdTag as StartTransaction.req. Verify that StopTransaction.conf is received with status 'Accepted'. Note: Step 7 and 8 can be in different order."
  },
  {
    step: 8,
    ev: "Send StatusNotification.conf (empty)",
