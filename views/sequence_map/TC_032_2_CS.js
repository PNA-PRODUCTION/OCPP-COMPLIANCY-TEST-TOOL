...
export const TC_032_2_CS_SEQUENCE = [
  {
    step: 1,
  ...
  },
  {
    ...
    description: "Verify that BootNotification.req is received from Charge Point after power restoration. Verify that BootNotification.conf is received with status 'Accepted'. Note: BootNotification is only allowed at step 6."
  },
  {
    step: 7,
    ev: "Send StatusNotification.conf (empty)",
    evse: "Send StatusNotification.req (connectorId = 0, status = Available)",
    description: "Verify that StatusNotification.req is received with connectorId = 0. Verify that status is 'Available'. Verify that StatusNotification.req is received with connectorId = 1 and status = 'Finishing'. Both connectorId 0 and 1 must be reported. Note: Step 7 and 8 can be in different order."
  },
  {
    step: 8,
    ev: "Send StopTransaction.conf",
    evse: "Send StopTransaction.req (reason = PowerLoss or Local)",
    description: "Verify that StopTransaction.req is received with reason 'PowerLoss' or 'Local'. Verify that StopTransaction.req contains the same IdTag as StartTransaction.req. Verify that StopTransaction.conf is received with status 'Accepted'. Note: Step 7 and 8 can be in different order."
  }
];
