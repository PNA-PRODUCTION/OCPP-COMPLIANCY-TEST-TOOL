...
export const TC_036_CS_SEQUENCE = [
  {
    step: 1,
  ...
  },
  {
    ...
    description: "Verify that 2 MeterValues.req messages are received. Verify that MeterValues interval matches MeterValueSampleInterval (tolerance ±1 second). Verify that sampledValue.context is 'Sample.Periodic'. Force Test ResetTimer to 80 seconds for reconnection test."
  },
  {
    step: 13,
    ev: "Send MeterValues.req (during disconnection)",
    evse: "Receive MeterValues.req",
    description: "Verify that 4 MeterValues data collected during the 65-second socket disconnection are received in one packet. Verify that the timestamps of each MeterValue in the packet are in chronological order."
  },
  {
    step: 14,
    ev: "Send MeterValues.req (2 messages after reconnection)",
    evse: "Receive MeterValues.req",
    description: "Verify that 2 MeterValues.req messages are received after reconnection. Verify that MeterValues interval matches MeterValueSampleInterval (tolerance ±1 second). Verify that sampledValue.context is 'Sample.Periodic'. Verify that MeterValues are received one by one normally after reconnection."
  },
  {
    step: 15,
