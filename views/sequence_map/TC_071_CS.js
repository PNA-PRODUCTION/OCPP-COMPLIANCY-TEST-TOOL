...
export const TC_071_CS_SEQUENCE = [
  {
    step: 1,
  ...
  {
    step: 3,
    ev: "Send ChangeConfiguration.req (MeterValueSampleInterval = 0)",
    evse: "Send ChangeConfiguration.conf",
    description: "If MeterValueSampleInterval is not 0, send ChangeConfiguration.req to set it to 0. Verify that ChangeConfiguration.conf is received with status 'Accepted'. If MeterValueSampleInterval is already 0, skip this step."
  },
  {
    step: 4,
  ...
  {
    step: 5,
    ev: "Send ChangeConfiguration.req (ClockAlignedDataInterval = 15)",
    evse: "Send ChangeConfiguration.conf",
    description: "If ClockAlignedDataInterval is not 15, send ChangeConfiguration.req to set it to 15. Verify that ChangeConfiguration.conf is received with status 'Accepted'. If ClockAlignedDataInterval is already 15, skip this step."
  },
  {
    step: 6,
    ev: "Send GetConfiguration.req (key = MeterValuesAlignedData)",
    evse: "Send GetConfiguration.conf",
    description: "Send GetConfiguration.req with key 'MeterValuesAlignedData'. Verify that GetConfiguration.conf is received and contains MeterValuesAlignedData value."
  },
  {
    step: 7,
  ...
  },
  {
    ...
    description: "Verify that 3 MeterValues.req messages are received. Verify that MeterValues interval matches ClockAlignedDataInterval (tolerance ±1 second). Verify that MeterValuesAlignedData configured values are included. If MeterValuesAlignedData value is empty, verify that default Energy.Active.Import.Register value is included. Verify that sampledValue.context is 'Sample.Clock'. Verify that MeterValue.timestamp contains a Clock-aligned value (e.g., for 15-second interval: seconds should be 0, 15, 30, 45)."
  },
  {
    step: 14,
